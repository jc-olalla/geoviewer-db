...
CREATE INDEX IF NOT EXISTS idx_viewers_slug_lower ON viewers (lower(slug));
...
-- 5) layers (GLOBAL catalog; no viewer_id here)
...
-- Helpful indexes that DO NOT reference viewer_id
CREATE UNIQUE INDEX IF NOT EXISTS uq_layers_name        ON layers (lower(name));
CREATE INDEX IF NOT EXISTS idx_layers_visible           ON layers (visible);
CREATE INDEX IF NOT EXISTS idx_layers_sort_order_id     ON layers (sort_order, id);
...
-- Optional: if you query bbox frequently as JSONB, keep this:
-- CREATE INDEX IF NOT EXISTS idx_layers_bbox_gin         ON layers USING GIN (bbox);
...
--------------------------------------------------------------------------------
-- 7) layer_permissions (viewers ↔ layers)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 8) styles (re-usable style JSON objects)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 9) layer_styles (bind a style to a layer)
--------------------------------------------------------------------------------
